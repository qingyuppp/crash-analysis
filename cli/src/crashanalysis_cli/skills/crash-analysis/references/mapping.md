...
`scripts/backtrace_resolve.py` inside the `crash-analysis/` skill directory.
...
```bash
python3 <path-to-crash-analysis>/scripts/backtrace_resolve.py /tmp/bt_input.json <report_dir>/backtrace.json
```
