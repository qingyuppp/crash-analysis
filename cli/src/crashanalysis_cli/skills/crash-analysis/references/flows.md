...
If the user supplied a vmcore, run `cra vmcore collect` followed by
`cra vmcore classify --collection <output-dir>/collection.json` before
considering the legacy Fetcher. The user need not classify the issue. Read `evidence.json`,
...
- Route XFS lock evidence without a primary crash signature to the **XFS hang
  ...
  `cra vmcore diagnose` actions for iterative `bt -f` and object queries.
...
enter it from a keyword match alone. Read `focus/xfs.txt`, then issue only the
 needed `cra vmcore diagnose` actions. Detailed evidence procedure:
...
3. **Classify direct waiters** — create one `bt <pid>` command for every
   ...
   `cra vmcore diagnose compact-bt` invocation, and read its compact
